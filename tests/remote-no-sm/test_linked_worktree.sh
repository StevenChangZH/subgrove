...
# inside a linked worktree, even on a remote-cloned no-submodule super.
# The refusal fires inside `assert_main_worktree` before any other side
# effect; this tier locks in that the assertion still fires when the
# super was cloned from a real remote.
#
# Companion to tests/local-no-sm/test_linked_worktree.sh — same idea,
# but on a wire-cloned super. State-preservation snapshot covers only
# the parent (no sm-a / sm-b).
set -eo pipefail
...
_run_linked_refusal() {
    ...
    mkfixture_remote_no_sm "linked_${subcmd}"
    cd "$FIXTURE_SUPER"
