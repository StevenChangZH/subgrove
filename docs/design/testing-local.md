...
| Build failure keeps the worktree | `BUILD_CHAIN=(sm-a)` + `BUILD_CMD` that commits on the parent worktree then `false`; `new feat-x` | new exits non-zero; **worktree kept**; `feat/feat-x` retained at the wip commit (1 ahead of `main`); `build failed in sm-a` / `worktree kept` warn; no `rolling back` | Build runs after the rollback is disarmed, so a build failure keeps the worktree + branch + any commits the build made; `status` still lists it. |
