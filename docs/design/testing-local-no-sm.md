...
3. **Submodule-relevant parameters never crash on a no-sm super.** `touch=<sm-name>`, multi-name `touch=` lists, `BUILD_CHAIN=(<sm>)`, `merge push=true` — every parameter that exists *because of* submodules either rolls back cleanly (setup failures), keeps the worktree with a warning (build failures), or produces a defined no-op. The tier pins each parameter's behavior so a future change can't silently break the degenerate case.
...
**A `BUILD_CHAIN` entry that doesn't exist on a no-sm super leaks the shell's `cd: No such file or directory` error** rather than a clean diagnostic — `cd $WT/<entry>` fails inside the guarded build step, which is treated as a build failure: the worktree is kept and `new` exits non-zero (no rollback). The `new_build_fail_keeps` test instead sets `BUILD_CHAIN=(.)` so the build actually runs (and fails) in the worktree root, pinning the keep-and-warn behavior directly rather than the leaky-`cd` path. Pretty-printing the `cd` error is a possible follow-up; out of scope for the tier itself.
...
| Build failure keeps the worktree | `.subgroverc` sets `BUILD_CHAIN=(.)` + `BUILD_CMD` that writes a marker then `false`; `new feat-bc` | new exits non-zero; **worktree + parent branch kept**; the build's `built-marker` survives in the worktree; `>>> Running build chain` + `build failed in .` / `worktree kept` appear; no `rolling back`; main super otherwise byte-identical | Build runs after the rollback is disarmed, so a build failure keeps the (structurally complete) worktree rather than tearing it down. Invariant 3; the no-sm counterpart of `local/`'s `new_build_fail_keeps`. |
